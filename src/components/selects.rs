...
pub fn SelectableSetting<T: ToHtml + PartialEq + Clone + 'static>(options: T, label: &'static str, on_change: EventHandler<String>) -> Element{
    rsx!{
        select {
            class: "w-full p-4 my-8 rounded-lg bg-transparent appearance-none text-gray-500 border border-slate-700 focus:outline-none focus:ring-0 peer hover:border-blue-500 focus:border-blue-500 text-center",
            onchange: move |evt| { on_change.call(evt.value()) },
