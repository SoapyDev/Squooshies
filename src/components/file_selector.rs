...
pub fn FileSelector(label: &'static str, value: PathBuf, on_change: EventHandler<String>, on_click: EventHandler<()>) -> Element {
    ...
    rsx!{
        div{
            class: "w-full my-8 flex flex-row flex-nowrap border border-slate-700 rounded-lg hover:border-blue-500 has-[:focus]:border-blue-500 transition-colors duration-250 ease-in-out",
            button{
